            ...
            <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
              {items.map((item, index) => (
                <motion.div
                  ...
                  className="p-4 sm:p-6 border-b border-stone-100 last:border-b-0"
                >
                  {/* Mobile Layout */}
                  <div className="flex gap-3 sm:gap-4">
                    {/* Product Image */}
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 bg-stone-100 rounded-xl overflow-hidden flex-shrink-0">
                      {item.imageUrl && item.imageUrl.startsWith('data:image') ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <svg
                            className="w-8 h-8 sm:w-12 sm:h-12 text-slate-300"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0">
                          ...
                          {item.attributes && (
                            <p className="text-xs sm:text-sm text-slate-500 truncate">{item.attributes}</p>
                          )}
                          <p className="text-sm sm:text-lg font-semibold text-slate-800 mt-1">
                        ...
                        <button
                          ...
                          className="text-red-500 hover:text-red-700 disabled:opacity-50 transition-colors flex-shrink-0"
                        >
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <div className="flex items-center gap-1 sm:gap-2">
                          <button
                            ...
                            className="w-7 h-7 sm:w-8 sm:h-8 rounded-full border border-stone-200 flex items-center justify-center hover:bg-stone-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                          >
                            <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" />
                            </svg>
                          </button>
                          <span className="w-8 sm:w-12 text-center font-medium text-slate-800 text-sm sm:text-base">
                            {isUpdating === `${item.skuId}` ? (
                              <svg className="animate-spin h-4 w-4 sm:h-5 sm:w-5 mx-auto text-amber-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                              </svg>
                            ) : (
